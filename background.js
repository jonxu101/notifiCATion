...
var domains = {},
    ...
    settings = {
        ...
        unproductiveTime: UNPRODUCTIVE_TIME_DEFAULT,
    ...
    STORAGE_UNPRODUCTIVE_TIME = "unproductive-time",
    ...
    loadTimeUnproductive = function() {
        return storageLocal.load(STORAGE_UNPRODUCTIVE_TIME, UNPRODUCTIVE_TIME_DEFAULT, function(e) {
            settings.unproductiveTime = e[STORAGE_UNPRODUCTIVE_TIME], saveTimeUnproductive(), dcl("Graph gap loaded: " + e[STORAGE_UNPRODUCTIVE_TIME])
        }), !0
    },
    saveTimeUnproductive = function() {
        return storageLocal.save(STORAGE_UNPRODUCTIVE_TIME, settings.unproductiveTime, function() {
            dcl("Unproductive time saved: " + settings.unproductiveTime)
        }), !0
    },
    setTimeUnproductive = function(e) {
        return settings.unproductiveTime = parseInt(e) || UNPRODUCTIVE_TIME_DEFAULT, !0
    },
    ...
    giveNotif = function(t) {
        // if (t > settings.unproductiveTime-1 && t < settings.unproductiveTime+1) {
        //     // alert("You have been on this website for "+settings.unproductiveTime+" minutes!");
        //     // alert("addingcat");

        //     // var port = chrome.tabs.connect(chrome.windows.getCurrent(function (currentWindow) {
        ...
        //     // port.postMessage({cmd: "addcatnow"});
        ...
        //     //     var port = chrome.tabs.connect(currentTabId, {name: "mycontentscript"})
        //     //     port.postMessage({cmd: "addcatnow"});
        //     //  });
        ...
        //     }, () => {
        //           connect("addcat1now") //this is where I call my function to establish a connection     });
        //       });
        ...
        //     //   });
        // }
        // var unproductiveDomains = [parseDomainFromUrl("https://www.youtube.com")];
        chrome.tabs.query({active: true, lastFocusedWindow: true}, tabs => {
            // var url = tabs[0].url;
            // var domain_url = parseDomainFromUrl(url);
            // unproductiveTime = unproductiveDomains[domain_url];
            // use `url` here inside the callback because it's asynchronous!
            // alert("CATTTT");
            if (t<=settings.unproductiveTime+5 && t >= settings.unproductiveTime){
                // alert("You have been on this website for "+settings.unproductiveTime+" minutes!");
                chrome.tabs.executeScript(null, {
                    file: 'content.js', //my content script   
                }, () => {
            ...
            }else if (t<=settings.unproductiveTime+10 && t > settings.unproductiveTime+5){
                chrome.tabs.executeScript(null, {
                    file: 'content.js', //my content script   
                }, () => {
                        connect("addcat2now") //this is where I call my function to establish a connection     });
                    });
            }else if (t<=settings.unproductiveTime+15 && t > settings.unproductiveTime+10){
                chrome.tabs.executeScript(null, {
                    file: 'content.js', //my content script   
                }, () => {
                        connect("addcat3now") //this is where I call my function to establish a connection     });
                    });
            }else if(t > settings.unproductiveTime+15 && t<=settings.unproductiveTime+20){
                chrome.tabs.executeScript(null, {
                    file: 'content.js', //my content script   
                }, () => {
                        connect("addcat4now") //this is where I call my function to establish a connection     });
                    });
            }else if(t>settings.unproductiveTime+20){
                chrome.tabs.executeScript(null, {
                    file: 'content.js', //my content script   
                }, () => {
                        connect("addcat5now") //this is where I call my function to establish a connection     });
                    });
            }
        });
        return !0;
        ...
        dates.today !== n && (dates.today = n, seconds.today = 0), chrome.windows.getLastFocused({
            ...
            for (var o in n.tabs)
                ...
                if (a = parseDomainFromUrl(s.url), t = parseProtocolFromUrl(s.url), (n.focused && "active" === o || e) && -1 === BLACKLIST_DOMAIN.indexOf(a) && -1 === BLACKLIST_PROTOCOL.indexOf(t) && "" !== a) {
                    ...
                    var i = domains[a];
                    i.days[dates.today] = i.days[dates.today] || getDayObj(), e || (i.alltime.seconds += INTERVAL_UPDATE_S, i.days[dates.today].seconds += INTERVAL_UPDATE_S, seconds.today += INTERVAL_UPDATE_S, seconds.alltime += INTERVAL_UPDATE_S, domainsChanged = !0), setBadge(d, getBadgeTimeString(i.days[dates.today].seconds)), giveNotif(i.days[dates.today].seconds)
                }
            })
        })
    };
...
chrome.tabs.onActivated.addListener(function(e) {
...
}), dcl("Webtime Tracker - background.js loaded"), loadDateStart(dates.today), loadSecondsAlltime(), loadIdleTime(), loadGraphGap(), loadTimeUnproductive(), loadBadgeDisplay(), loadScreenshotInstructionsRead(), loadDomains(function(e) {
    ...
    updateDomains()
}, INTERVAL_UPDATE_MS), timeIntervals.save = window.setInterval(function() {
